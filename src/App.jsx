...
import { Cards, Navbar } from "./Components/services";
import  {Quot,Viewcards} from './Components/qoutation'
...
function App() {
  ...
  let searchItem = (val) => {
    setselectcat(val);
    let filterList = list.filter((x) => x.category == val);
  ...
  let filterResult = (value) => {
    if (result == "") {
      const result = list.filter((x) => x.category == value);
      setResult([...result]);
    } else {
      setResult([]);
    }
  ...
  return (
    ...
    <img width={200} src="logo.png" style={{width:"20%", height:"40px", marginRight:"10px"}} />
      <input className="input1" style={{minWidth:"50%",maxWidth:"60%",height:"30px", border:"2px solid #FF6A00" , borderRadius:"20px 0px 0px 20px", paddingLeft:"10px"}} placeholder="Searching The Item......"/>
      <button style={{height:"36px", width:60, border:"none", backgroundColor:"#FF6A00",color:"#FFFFFF" , borderRadius:"0px 20px 20px 0px"}}>Search</button>
            ...
            <Grid item md={9} xs={12} sm={9} sx={{overflow:"hidden"}}>
              <Box  sx={{
                ...
                {Categories.map((e, i) => (
                  <button
                    className="mainbtn"
                    style={{cursor:"pointer", height: "40px", width: "90px", margin: "5px 10px" , borderRadius:"20px"}}
                    key={i}
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {e.description.slice(0, 20) +
                      (e.description.length > 20 ? "..." : "")}
                       ...
                       {/* //////////Cards//// */}
        <Grid container spacing={1} maxWidth="xlg">
          <Grid item md={4} sm={6} xs={12}>
            <Container >
              <Card sx={{ padding: "5px", height: "250px" ,margin:"10px 0px"}}>
                <CardContent sx={{ textAlign: "left" }}>
                  <Viewcards 
                    link={
        ...
        <Container maxWidth="lg" className="quot">
        <Quot />
        </Container>
        <Container maxWidth="lg" >
          <Typography
